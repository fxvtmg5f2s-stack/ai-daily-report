...
输出Markdown（严格模板，总长1500~1800字，日期统一 YYYY-MM-DD，板块标题与内容之间空一行。所有链接一律内嵌在标题里，格式为 **[标题](url)**，禁止单独输出链接行；无 url 的条目标题不带链接）：
# 📰 AI 今日动态 — ${new Date().toISOString().slice(0, 10)}
## 📝 今日看点
（3-5条客观趋势归纳，每条一行，不要链接）
## 🏆 今日必读
（3条；至少1条来源不是OpenAI；优先「多源交叉」；每条三行：
**📌 [标题](url)**（来源·YYYY-MM-DD）
> 摘要
💡 科普：…）
## 🏢 公司动态
### 公司名
